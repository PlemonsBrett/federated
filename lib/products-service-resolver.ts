...
// Handler resolving the entities from represenations argument
      ...
      switch (event.info.fieldName) {
        case "product":
          if (event.arguments.id)
            result = products.find((p) => p.id === event.arguments.id);
          if (event.arguments.sku && event.arguments.package)
            result = products.find(
              (p) =>
                (p.sku === event.arguments.sku && p.package) ===
                event.arguments.package
            );
          if (
            event.arguments.sku &&
            event.arguments.variation &&
            event.arguments.variation.id
          )
            result = products.find(
              (p) =>
                p.sku === event.arguments.sku &&
                p.variation.id === event.arguments.variation.id
            );
          break;
        case "_service":
          ...
          for (const representation of representations as [any]) {
            const filteredProduct = products.find((p: any) => {
              for (const key of Object.keys(representation)) {
                if (
                  typeof representation[key] != "object" &&
                  key != "__typename" &&
                  p[key] != representation[key]
                ) {
                  return false;
